...
log("import_renpy %s %s" % (good, err))
good, miss, err, extra = boot.stage_import_all()
log("import_all %s %s" % (good, err))
...
from renpy.display.transition import Fade, Dissolve, MultipleTransition
...
log("Solid rv size=%s reverse=%s children=%s" % (
    srv.get_size(),
...
log("Solid _child_to_texture -> %s handle=%s size=%sx%s" % (
...
log("Solid direct draw mean_rgba=(%.1f,%.1f,%.1f,%.1f)" % m)

# Now Fade with product Solid, hold=0 (product default)
for hold in (0.0, 0.1):
    log("=== Fade hold=%s with product Solid ===" % hold)
    f = Fade(0.5, hold, 0.5, old_widget=old, new_widget=new)  # uses default Solid black
    log("Fade type=%s delay=%s n_trans=%s" % (type(f).__name__, f.delay, len(f.transitions)))
    ...
    for name, st in stages:
        ...
        log("  mean=(%.1f,%.1f,%.1f,%.1f) luma=%.1f dL=%s" % (m[0],m[1],m[2],m[3], L, dL))
        prev_l = L
...
for st in (0.0, 0.125, 0.25, 0.375, 0.5):
    ...
    if slots:
        for s in slots:
            sizes.append("%sx%s" % (getattr(s,'width',None), getattr(s,'height',None)))
