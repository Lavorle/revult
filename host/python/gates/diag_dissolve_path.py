...
try:
    ...
    def _dml(node, ox=0.0, oy=0.0):
        ...
        hits["leaf_draw"]+=1
        rec("draw_model_like shaders=%s texs=%s uniforms=%s"%(shaders, len(texs) if texs else None, getattr(node,"uniforms",None)))
    ...
    def _ctt(child):
        t=_orig_ctt(child)
        rec("child_to_texture type=%s -> %s handle=%s"%(type(child).__name__, type(t).__name__ if t else None, getattr(t,"handle",None)))
        return t
    ...
    rec("ROOT mean=(%.1f,%.1f,%.1f) hits=%s"%(rs/cnt,gs/cnt,bs/cnt,hits))
except Exception as e:
    rec("EXC %s"%e); rec(traceback.format_exc())
out.write_text("\n".join(lines)+"\n")
try:
