...
try:
    ...
    if not path:
        ...
        log(
            "budget max_frames={} w={} h={} fps={} layout={}x{} warm={}".format(
        ...
        while (time.monotonic() - t_first0) < 5.0:
            ...
            time.sleep(0.01)
        log(
            "first_present_probe frame_index={} nframes={} wall_s={:.3f}".format(
                first_idx_probe,
        ...
        if first_idx_probe is None:
            ...
            ok = False
            log(
                f"FAIL first frame_index={first_idx_probe} != 0 (AC2 start-at-zero; "
                "allow ≤2 for scheduling slack)"
            )
        else:
            log(f"PASS first frame_index == 0 (got {first_idx_probe})")
        ...
        log(
            f"wait_ready_full elapsed_s={wait_full_elapsed:.3f} full_ok={full_ok} budget_s={wait_full_s:.1f}"
        )
        ...
        if decode_fps < 29.0:
            ok = False
            log(f"FAIL decode_fps={decode_fps:.3f} < 29 (product 30fps parity)")
        else:
            log(f"PASS decode_fps={decode_fps:.3f} >= 29")
        ...
        if _has_helper("path_cache_ready_full"):
            ...
            log(f"ch.ready_full={ready_full}")
        else:
        ...
        if (
            ...
            log(
                ...
                f"pre-decode (pos={pos_after_play:.4f} play_wall={t_play:.3f})"
            )
        else:
            log(
                "PASS clock_after_ready "
                f"(play_wall={t_play:.3f}; early playable-prefix arm is OK)"
            )
        ...
        if surf is None:
            ...
            log(f"post_full_frame_index={idx_now} (informational; AC2 checked at first present)")
            ...
            if nframes > 0 and empty_presents != 0:
                ok = False
                log(
                    f"FAIL empty_movie_presents={empty_presents} != 0 while frames exist (AC1)"
                )
            elif nframes > 0:
        ...
        if _has_helper("path_cache_has_frames") or _has_helper("path_cache_frame_count"):
            if not survived and nframes2 < 360:
                ...
                log(f"PASS path cache survives stop→play (survived={survived})")
        else:
            ...
            if nframes2 >= 360 and t_replay < max(0.05, t_play * 0.25):
                ...
                warn(
                    f"stop→play re-play wall={t_replay:.3f} (first={t_play:.3f}); "
        ...
        if surf2 is not None:
            ...
            log(f"second_play frame_index={idx2}")
            if idx2 is not None and int(idx2) > 2:
                ok = False
                log(f"FAIL second_play frame_index={idx2} > 2 (expected near 0)")
            else:
                log(f"PASS second_play frame_index near 0 ({idx2})")
        else:
            ok = False
