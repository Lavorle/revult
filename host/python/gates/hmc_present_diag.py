...
def _log(msg):
    try:
        sys.__stdout__.write("[present_diag] %s\n" % msg); sys.__stdout__.flush()
    except Exception:
        pass
    ...
    def watcher():
        ...
        while ticks<200 and not done["v"]:
            ...
            try:
                ...
                if not mm:
                    if ticks%15==0: _log("wait mm t=%s" % ticks)
                    continue
                if saw_mm_at is None:
                    saw_mm_at=ticks
                    _log("main_menu at t=%s" % ticks)
                ...
                tt=dict(getattr(iface,"transition_time",{}) or {}) if iface else {}
                ft=getattr(iface,"frame_time",None) if iface else None
                age=ticks-saw_mm_at
                _log("age=%.1fs mean=%s diss=%s ot=%s ft=%s" % (age*0.2, mean, diss, ot, ft))
                ...
                if age==15:
                    ...
                    _log("force_live mean=%s diss=%s" % (_dock_mean(), _find_dissolve(draw, getattr(iface,"surftree",None))))
                if age==18:
                    try:
                        import interact_helpers as ih
                        ready,why,iface2=ih.interface_ready()
                        ...
                        _log("rebuild mean=%s" % (_dock_mean(),))
