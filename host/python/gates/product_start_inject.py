...
Writes host/target/gate-product-start-inject.txt
"""
import os
import sys
import threading
...
def run():
    base = _base()
    out = base / "host" / "target" / "gate-product-start-inject.txt"
    lines = []
    ...
    def injector():
        # Wait for main menu to paint + before_interact focus seed (first Button=Start).
        time.sleep(5.0)
        state["phase"] = "injecting"
        rec("begin inject Enter-only for Start (seed focuses first Button)")
        try:
            import renpy

            K_RETURN = 13
            for i in range(30):
                # Enter only — before_interact seeds first nav Button (Start).
                # Do not inject Down first (that moves focus to Load).
