        ...
        def _limited_interact(*a, **k):
            ...
            if state.get("menu_label") and label_after is not None:
                try:
                    ...
                    if isinstance(label_after, tuple) and len(label_after) >= 2:
                        after_serial = int(label_after[1]) if False else label_after
                except Exception:
                    pass
            ...
            if (
                ...
                if state.get("hit_bad_ending_text") or (
                    ...
                    _append_log(
                        log,
                        "BAD_ENDING_REACHED path=%s label=%r say=%r forced_picks=%s "
                        "later_dlg=%s bad_text=%s"
                        % (
                            path,
                            label_after,
                            (say_blob or "")[:240],
        ...
        if _looks_like_later(label, say_blob, state):
            ...
            if not any("BAD_ENDING_REACHED" in x for x in log):
                ...
                _append_log(
                    log,
                    "BAD_ENDING_REACHED path=%s label=%r say=%r forced_picks=%s"
                    % (path, label, (say_blob or "")[:200], state.get("forced_second_picks")),
                )
