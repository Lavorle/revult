"""
AC-C1 tq_chrome_live gate — product chrome stack (main menu + real Frame).

Gate name: tq_chrome_live  (RENPY_HOST_GATE=tq_chrome_live)

Contract (product visual LIVE restore / AC-C1):
  - Product main menu and/or confirm **stack** capture required
    (HostInterface / interact_helpers / real screen path).
  - Isolated FakeRender multipiece alone is NOT sufficient for AC-C1.
  - Orange border bands ≈(204,102,0)±tol on frame panels.
  - Center black OK by design (gui/frame.png solid black center).
  - Do NOT set prefs.transitions=0 (leave default / ≥2).
  - less_updates=False, models=True logged as AC-L0 chrome-adjacent probe.

Probe0 (log-only, before pass/fail):
  - Sample product main-menu RT: orange present? featureless black?
  - Main menu uses overlay/main_menu.png (not frame.png) — orange may be
    absent by design on bare main menu; document that honestly.

AC-C1 pass path:
  - After product main menu is live under HostInterface, render a real
    renpy.display.imagelike.Frame("gui/frame.png", Borders(40,...)) via
    renpy.display.render.render, draw through product WgpuDraw, sample
    orange border structure on the product RT.
...
GATE_NAME = "tq_chrome_live"

# Product confirm chrome borders (gui.confirm_frame_borders = Borders(40,40,40,40)).
LEFT = TOP = RIGHT = BOTTOM = 40
DST_W, DST_H = 480, 280
OX, OY = 120, 100  # placement on 1280x720 virtual
...
def _append_log(lines: list, msg: str) -> None:
    ...
    print(f"[tq_chrome_live] {msg}", flush=True)


def _report_path(base: Path) -> Path:
    return base / "host" / "target" / "gate-tq_chrome_live.txt"
...
def _write_report(base: Path, meta: dict, log: list) -> Path:
    ...
    lines = [
        ...
        f"loadable_frame={meta.get('loadable_frame')}",
        f"loadable_main_menu={meta.get('loadable_main_menu')}",
        # AC-L0-adjacent (chrome live must not force transitions=0)
        ...
        # Probe0
        f"probe0_orange_hits={probe0.get('orange_hits')}",
        ...
        f"probe0_note={probe0.get('note')}",
        # AC-C1 chrome structure
        f"chrome_border_hits={chrome.get('border_hits')}",
        f"chrome_border_ok={chrome.get('border_ok')}",
        f"chrome_center_rgb={chrome.get('center_rgb')}",
        f"chrome_center_black={chrome.get('center_black')}",
        f"chrome_center_orange={chrome.get('center_orange')}",
        f"chrome_featureless_black={chrome.get('featureless_black')}",
        f"chrome_top={chrome.get('top')}",
        f"chrome_left={chrome.get('left')}",
        f"chrome_path={chrome.get('path')}",
        f"chrome_ok={chrome.get('ok')}",
...
def _probe0_main_menu(rgba, rw, rh, log: list) -> dict:
    ...
    _append_log(
        log,
        "Probe0 main_menu orange_hits=%s/%s featureless_black=%s mean=%s var=%s nonblank=%s"
        % (
            orange_hits,
            samples,
            featureless_black,
...
def _read_l0_state(log: list) -> dict:
    ...
    try:
        ...
        if prefs is not None and hasattr(prefs, "transitions"):
            out["transitions_pref"] = int(getattr(prefs, "transitions"))
    ...
    _append_log(
        log,
        "AC-L0-adj less_updates=%s models=%s transitions_pref=%s forced_zero=%s"
        % (
            out["less_updates"],
            out["models"],
...
def _draw_product_frame_chrome(log: list) -> dict:
    ...
    try:
        ...
        import renpy.display.imagelike as imagelike
        import renpy_host  # type: ignore
        import interact_helpers as ih
...
def _install_hooks(state: dict, log: list, max_interacts: int) -> None:
        ...
        def draw_screen(self, root_widget, fullscreen_video, draw):
            ...
            if (
                ...
                if callable(capturer):
                    ...
                    raise HostStop(
                        "chrome_live",
                        "captured ownership_ok=%s chrome_ok=%s path=%s"
                        % (
                            state.get("ownership_ok"),
                            (state.get("chrome") or {}).get("ok"),
                            (state.get("chrome") or {}).get("path"),
                        ),
                    )
    ...
    def _host_run(restart):
        ...
        try:
            ...
            tp = None
            if prefs is not None and hasattr(prefs, "transitions"):
                tp = int(getattr(prefs, "transitions"))
            _append_log(
                log,
        ...
        def _do_capture(tag: str) -> None:
            ...
            try:
                ...
                try:
                    ...
                    _append_log(
                        log,
                        "main_menu ownership_ok=%s size=%sx%s path=%s"
                        % (
                            state.get("ownership_ok"),
                            state.get("frame_w"),
                            state.get("frame_h"),
                ...
                # --- AC-C1: product Frame chrome stack ---
                chrome = _draw_product_frame_chrome(log)
                state["chrome"] = chrome
                # Prefer chrome RT dimensions when available.
                if chrome.get("frame_w"):
                    state["frame_w"] = chrome.get("frame_w")
                    state["frame_h"] = chrome.get("frame_h")
                    state["frame_ok"] = True
                    state["frame_bytes"] = int(chrome["frame_w"]) * int(chrome["frame_h"]) * 4
                # Re-read ownership after chrome present.
                try:
                    own2 = ih.read_present_ownership()
                    state["ownership"] = own2
                    state["ownership_ok"] = (
                        bool(own2.get("last_product_present"))
                        and int(own2.get("product_presents") or 0) >= 1
                        and int(own2.get("idle_clears_after_present") or 0) == 0
                        and not own2.get("error")
                    ) or bool(state.get("ownership_ok"))
                    if chrome.get("path"):
                        state["present_path"] = chrome.get("path")
                except Exception as e:
                    _append_log(log, f"post-chrome ownership soft-fail: {e}")
...
def run() -> None:
    ...
    def _atexit_report():
        ...
        try:
            ...
            meta["chrome"] = state.get("chrome") or {}
    ...
    try:
        ...
        chrome_ok = bool(chrome.get("ok"))
        probe0 = state.get("probe0") or {}
        frame_ok = bool(state.get("frame_ok"))

        # SSOT loadable checks (product assets).
        loadable_frame = False
        loadable_main_menu = False
        loadable_err = ""
        try:
            ...
            loadable_frame = bool(_loader.loadable("gui/frame.png"))
            loadable_main_menu = bool(_loader.loadable("gui/main_menu.png"))
        except Exception as e:
            loadable_err = f"{type(e).__name__}: {e}"
        _append_log(
            log,
            f"loadable frame={loadable_frame} main_menu={loadable_main_menu}"
            + (f" err={loadable_err}" if loadable_err else ""),
        )

        # Fail-closed ok: product stack reached + chrome orange structure + loadable.
        # Main menu in_mm is preferred; chrome_ok is the AC-C1 structure assert.
        # ownership_ok preferred but chrome path alone may still present via product draw.
        transitions_forced_zero = bool(state.get("transitions_forced_zero"))
        ok = bool(
            chrome_ok
            and loadable_frame
            and not transitions_forced_zero
            and (in_mm or ownership_ok or (chrome.get("path") or "").startswith("product_"))
        )
        ...
        if not ownership_ok:
            notes_parts.append(
                "ownership_soft last=%s presents=%s idle_clears=%s err=%s"
                % (
        ...
        if not chrome_ok:
            notes_parts.append(
                "chrome_fail border_hits=%s featureless=%s err=%s top=%s left=%s path=%s"
                % (
                    chrome.get("border_hits"),
                    chrome.get("featureless_black"),
                    chrome.get("error"),
                    chrome.get("top"),
                    chrome.get("left"),
                    chrome.get("path"),
        ...
        if not loadable_frame:
            notes_parts.append(
                "loadable_frame_fail path=gui/frame.png"
                + (f" err={loadable_err}" if loadable_err else "")
            )
        ...
        if ok:
            notes_parts.append(
                "AC-C1 product Frame chrome orange border structure under product draw; "
                "center black OK by design; transitions not forced 0"
            )
        if state.get("capture_error"):
            notes_parts.append(f"capture_error={state.get('capture_error')}")
        if chrome.get("error"):
            notes_parts.append(f"chrome_error={chrome.get('error')}")

        # Probe0 residual hint: bare main menu featureless black is a present bug;
        # orange-absent alone is expected (overlay, not frame.png).
        if probe0.get("featureless_black"):
            notes_parts.append(
                "PROBE0: main menu RT featureless black — present/draw residual"
            )
        elif int(probe0.get("orange_hits") or 0) == 0:
            notes_parts.append(
                "PROBE0: no orange on bare main menu (expected: overlay not frame.png)"
            )
        ...
        meta.update(
            {
                ...
                "frame_ok": frame_ok,
                "frame_error": state.get("frame_error") or chrome.get("error") or "",
                "present_path": state.get("present_path") or chrome.get("path") or "",
                "interact_count": state.get("interact_count"),
                "loadable_frame": loadable_frame,
        ...
        _append_log(
            ...
            f"chrome_ok={chrome_ok} border_hits={chrome.get('border_hits')} "
            f"transitions_pref={meta.get('transitions_pref')} "
        ...
        if not ok:
            raise RuntimeError(
                f"{GATE_NAME} ok=False notes={meta['notes']}; see gate-tq_chrome_live.txt"
            )
