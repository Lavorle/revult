...
def _analyze_nav(rgba, rw, rh, log: list) -> dict:
    ...
    _append_log(
        log,
        "nav pure_black=%d/%d frac=%.3f arena_clear=%d nav_ok=%s overlay=%s scenic=%s "
        ...
        def draw_screen(self, root_widget, fullscreen_video, draw):
            ...
            if (
                ...
                if callable(capturer):
                    ...
                    raise HostStop(
                        "nav_transparent",
                        "captured ownership_ok=%s nav_ok=%s pure_frac=%s"
                        % (
...
def run() -> None:
    ...
    try:
        ...
        if not nav_ok:
            notes_parts.append(
                "nav_fail pure_frac=%s pure_black=%s/%s arena_clear=%s err=%s"
                % (
                    nav.get("pure_frac"),
                    nav.get("pure_black"),
