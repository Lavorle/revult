...
export RENPY_HOST_GATE=hmc_menu_video_product
export RENPY_HOST_PHASE0_SIGNALS=1
export RENPY_HOST_SMOKE_SECS=90
...
: > /tmp/hmc_menu_video_product.log
: > "$ROOT/host/target/gate-hmc_menu_video_product.txt"
: > "$ROOT/host/target/gate-hmc_menu_video_product.json"
mkdir -p .omx/tmp/wp0
ls "$RENPY_HOST_GAME/game" | head
echo "=== launching menu video product gate ==="
"$ROOT/host/target/release/renpy-host" "$RENPY_HOST_GAME" > .omx/tmp/wp0/menu-video-product-stdout.log 2> .omx/tmp/wp0/menu-video-product-stderr.log
echo "exit=$?"
echo "--- gate artifact ---"
cat host/target/gate-hmc_menu_video_product.txt 2>/dev/null || echo "no artifact"
echo "--- json summary ---"
python3 -c "import json; from pathlib import Path; p=Path('host/target/gate-hmc_menu_video_product.json'); d=json.loads(p.read_text()) if p.exists() and p.stat().st_size else None; print('no json' if not d else 'ok');
import sys
" 2>/dev/null || true
python3 <<'PY2'
import json
from pathlib import Path
p = Path("host/target/gate-hmc_menu_video_product.json")
...
PY2
echo "--- log tail ---"
tail -100 /tmp/hmc_menu_video_product.log 2>/dev/null || true
echo "--- stderr tail ---"
tail -50 .omx/tmp/wp0/menu-video-product-stderr.log 2>/dev/null || true
